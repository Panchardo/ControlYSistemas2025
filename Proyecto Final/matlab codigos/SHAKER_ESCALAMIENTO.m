...
%%
co = ctrb(A_aum_p, B_aum_p(:,1));  % matriz de controlabilidad
rank_co = rank(co);


ob = obsv(A_aum_p, C_aum_p(8,:)); % Por que usar C_aument(8,:), que es la fila de la aceleracion que mido, hace que dé rango 2?
rank_ob = rank(ob);
...
A_reduc = A_aum_p(idx_obs, idx_obs);      % 3x3
B_reduc = B_aum_p(idx_obs,1);               % 3x1
...
p_obs_int = [-5000 -5000 -100000 -R/L*100]; 
