...
[f1, mag_s, phase_s ] = my_dft(adc_signal_f, Fs);
[f2, maf_f, phase_f ] = my_dft(iir_output, Fs);
[f3, maf_m, phase_m ] = my_dft(iir_output_m, Fs);

hfvt = fvtool(Hd, 'Analysis','Phase');
...
% figure
% plot(f1, angle(phase_s));
% hold on;
% plot(f2, angle(phase_f));
% grid on;
% plot(f3, angle(phase_m));
% legend('Señal','C', 'MATLAB')
