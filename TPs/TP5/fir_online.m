...
adc_signal = sin(2 * pi * Fn1 * t) + sin(2 * pi * Fn2 * t) + 0.25 * sin(2 * pi * Fn3 * t); %+ sin(2*pi*900*t);

% Input signal must be in float, not double
adc_signal_f = single(adc_signal); %% eso es porque se va a usar en fir_wrapper.c, que utiliza floats en vez de doubles como MATLAB
